...
from morphir.ir.decorations import (
    DecorationFormat, LayerManifest, DecorationValuesFile, 
    SchemaRef, merge_decoration_values, deep_merge
)
...
class TestDecorations:
    def test_schema_ref(self):
        ref = SchemaRef(
            display_name="Docs",
            local_path="schemas/doc.json",
            entry_point="Doc#Main"
        )
        assert ref.display_name == "Docs"
    
    def test_decoration_format(self):
        fmt = DecorationFormat(
    ...
    def test_merge_decoration_values(self):
        ...
        # Layer 10 (Override)
        layer10 = {
            "node1": {"summary": "Override Summary", "details": ["extra"]}, # details should merge? dict merge logic check
            # deep_merge logic:
            # list + list -> concat
        ...
        merged = merge_decoration_values([
            (0, layer0),
            (10, layer10)
        ])
